...
Usage: Run on both primary and standby and compare outputs. Safe for pgAdmin and psql.
...
SELECT
    ...
    NULL::numeric AS write_lag_seconds,
    NULL::numeric AS flush_lag_seconds,
    CASE
...
-- SAMPLE_OUTPUT_BEGIN
-- node_role | database_name | application_name | client_addr | state | sync_state | sent_lsn | write_lsn | flush_lsn | replay_lsn | send_replay_gap_bytes | replay_lag_seconds | rpo_status
-- ----------+---------------+------------------+-------------+-------+------------+----------+-----------+-----------+------------+-----------------------+--------------------+------------
-- (0 rows)
-- SAMPLE_OUTPUT_END
