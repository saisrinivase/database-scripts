/*
Oracle DBA Script: Database Growth AWR Daily
Purpose: Aggregate all AWR tablespace usage into a daily database growth trend.
Area: Database Size
...
PROMPT Database Growth AWR Daily
...
WITH snap_day AS (
    ...
    SELECT sd.snap_date,
           SUM(tsu.tablespace_size * dt.block_size) AS allocated_bytes,
           SUM(tsu.tablespace_usedsize * dt.block_size) AS used_bytes,
           SUM(tsu.tablespace_maxsize * dt.block_size) AS max_bytes
...
SELECT snap_date,
       ...
       ROUND((used_bytes - LAG(used_bytes) OVER (ORDER BY snap_date))/1024/1024/1024, 2) AS daily_used_growth_gb,
       ROUND((allocated_bytes - LAG(allocated_bytes) OVER (ORDER BY snap_date))/1024/1024/1024, 2) AS daily_alloc_growth_gb
FROM daily
ORDER BY snap_date;
