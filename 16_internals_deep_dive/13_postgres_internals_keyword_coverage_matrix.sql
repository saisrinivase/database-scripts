...
FROM (VALUES
    ...
    ('WAL', 'wal_internals', '13_io_wal_checkpoints/04_wal_archiver_health.sql', '33_bgwriter_memory_pressure/02_wal_writer_archiver_pressure.sql; 38_observability_360/07_wal_checkpoint_archiver_dashboard.sql', 'Track WAL generation, archiver failures, WAL buffers, and flush pressure.'),
    ('checkpoint/bgwriter/checkpointer', 'write_path', '10_maintenance_monitoring/01_bgwriter_checkpoint_stats.sql', '13_io_wal_checkpoints/05_checkpoint_pressure_indicators.sql; 26_physical_cloud_diagnostics/03_checkpoint_fsync_pressure.sql', 'Diagnose checkpoint frequency, sync/write time, dirty buffers, and backend writes.'),
    ...
    ('autovacuum/vacuum freeze', 'maintenance_internals', '07_vacuum_bloat/05_vacuum_progress.sql', '33_bgwriter_memory_pressure/03_autovacuum_worker_pressure.sql; 07_vacuum_bloat/04_dead_tuples_hotspots.sql', 'Find running vacuum progress, worker saturation, dead tuples, and freeze risk.'),
    ('bloat/dead tuples', 'storage_health', '07_vacuum_bloat/01_table_bloat_estimate.sql', '07_vacuum_bloat/04_dead_tuples_hotspots.sql; 03_index_analysis/05_index_bloat_estimate.sql', 'Estimate bloat and prioritize vacuum/reindex/table rewrite action.'),
    ('planner/stats/selectivity', 'planner_internals', '12_planner_statistics/01_tables_needing_analyze.sql', '12_planner_statistics/02_seq_scan_hotspots.sql; 12_planner_statistics/04_extended_statistics_candidates.sql', 'Find stale stats, bad estimates, missing extended stats, and seq scan hotspots.'),
    ('pg_stat_statements/top SQL', 'query_attribution', '10_maintenance_monitoring/03_top_statements_pg_stat_statements.sql', '28_pgss_resource_attribution/01_pgss_cpu_io_memory_rankings.sql; 38_observability_360/10_query_capture_quality_pgss.sql', 'Rank CPU/time/IO/temp/WAL SQL and verify capture quality.'),
