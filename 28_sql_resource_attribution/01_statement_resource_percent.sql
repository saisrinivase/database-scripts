/*
MySQL DBA Script: Statement Resource Percent
Purpose: Provide MySQL DBA diagnostics for statement resource percent.
Area: Sql Resource Attribution
Usage: Run with the mysql client or MySQL Shell in SQL mode as a user with privileges to read information_schema, performance_schema, sys, and mysql metadata where referenced.
Notes: Review findings before taking action. Some scripts require performance_schema consumers/instruments to be enabled.
*/
/* MySQL client settings: run with mysql, MySQL Shell SQL mode, or a compatible client. */
SELECT CONCAT('Running: Statement Resource Percent') AS script_name;
